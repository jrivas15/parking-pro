...
def seed(apps, schema_editor):
    ...
    Municipio.objects.bulk_create([Municipio(**m) for m in MUNICIPIOS])


def unseed(apps, schema_editor):
    Municipio = apps.get_model('municipios', 'Municipio')
    codes = [m['code_municipio'] for m in MUNICIPIOS]
    Municipio.objects.filter(code_municipio__in=codes).delete()
